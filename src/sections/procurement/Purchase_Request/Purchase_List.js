...
//console.log("response__d",response)
  ...
  const getb = useCallback(async () => {
    ...
    // console.log("response__getdb___",response);
      if (
        response.data.data &&
...
const fetchDataUsingRefreshBtn = useCallback(async () =>{
  ...
  const handleEditRow = useCallback(
    (id,row) => {
//console.log("row______",row);
      const Rowid = id;
      const AstNo = row.col1;
     
      if (Rowid !== '' && AstNo !== '') {
        navigate(`/dashboard/PurchaseRequest/editpurchaserequest`, {
          state: {
            RowID:Rowid,
            Ast_no:AstNo,
            currentPage,
          ...
          <CustomBreadcrumbs
            ...
            action={
              <Button
                ...
                to={{
                  pathname: "/dashboard/PurchaseRequest/newpurchaserequest",
                  state: { select: "New_PR" },
                }}
                               ...
                               // .slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                                .map((row) => (
                                  <MTRTableRow
                                    ...
                                    selected={table.selected.includes(row.col27)}
                                    onSelectRow={() => table.onSelectRow(row.col27)}
                                    onDeleteRow={() => handleDeleteRow(row.col27,row)}
                                    onEditRow={() => handleEditRow(row.col27,row)}
                                    onDuplicateRow = {() => handleDuplicateRow(row.col27,row)} 
                                    onPrintQrCode = {() => handlePrintQr(row.col27,row)} 
                                    onClick={() => handleRowClickTable(row.col27,row)}
