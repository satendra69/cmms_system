...
const PreventiveMaintenanceCalendarView = () => {
...
const { selectDropRowID: returnedSelectedOption, selectedOptionBack,comeBack ,selectedRowIdBack} = location.state || {};
  ...
  const fetchEvents = async () => {
    try {
      const response = await httpCommon.get(
        `/get_preventive_maintenance_calender_data.php?site_cd=${site_ID}`
      );
    //  console.log("response_____calendar",response);
      const formattedEvents = response.data.data.CalendarData.map((item) => ({
        start: new Date(item.prm_mst_create_date),
        end: new Date(item.prm_mst_pm_date),
        title: `${item.prm_mst_pm_no}`,
        status: `${item.prm_mst_type}`,
        rowId: `${item.RowID}`,
      }));
...
const handleCloseWorkQryList = () => {
  ...
  //  console.log("inputValueGet____",inputValueGet);
    if (inputValueGet !== "" && inputValueGet !== null) {
      ...
      try {
        const response = await httpCommon.get(
          `/get_SearchPreventiveMaintenanceModule.php?site_cd=${site_ID}&searchTerm=${inputValueGet}`
        );
   ...
   // console.log("enter___Fetching data..");
    try {
      ...
      const response = await httpCommon.post(
        ...
        console.log("response___getb2",response);
      if (
        response.data.data &&
        response.data.data.result &&
        response.data.data.result.length > 0
...
//console.log("response____fetchDataResponse",response);
      setSelectedOption(response.data.titleName);
  ...
  const handleDropDownPromptSaveAsBtn = async () => {
    ...
    if (hasEmptyValuept) {
      ...
      //console.log("insert__combinedData",combinedData);
      try {
        const response = await httpCommon.post(
          "/insert_preventive_maintenance_query_list_prompt_save_data.php",
          combinedData
        );
       //   console.log("insert__response",response);
        if (response.data.status == "SUCCESS") {
         // Swal.close();
...
PreventiveMaintenanceCalendarView.propTypes = {
  data: PropTypes.shape({
    site_cd: PropTypes.string.isRequired,
  }).isRequired,
};
export default PreventiveMaintenanceCalendarView;
