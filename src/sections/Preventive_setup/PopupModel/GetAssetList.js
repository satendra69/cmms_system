...
const GetAssetList = ({ onRowClick, onChangePage, onSearchChange }) => {
  ...
  useEffect(() => {
    ...
    Swal.showLoading();
    httpCommon
      .get("/get_asset_pm_list_model.php?site_cd=" + site_ID)
      .then((response) => {
  ...
  const handleSearch = (searchText) => {
    ...
    const filteredData = data.filter((item) => {
      ...
      const longDesc = item.ast_mst_asset_longdesc ? item.ast_mst_asset_longdesc.toLowerCase() : "";
      ...
      return (
        ...
        longDesc.includes(searchText.toLowerCase()) ||
