...
const DefaultLanguageTable = ({
  ...
  useEffect(() => {
    ...
    httpCommon
      ...
      .then((response) => {
        console.log("response__Template", response);
  ...
  const columns = [
    ...
    { title: "Template Description", field: "template_descs", width: 40 },
  ];

  const renderIcon = (rowData) => {
    if (rowData.template_cd === selectedRowKeys) {
      return (
        <div>
          <Iconify
    ...
    return <div>{rowData.template_cd}</div>;
  };
  const handleRowClick = (event, rowData) => {
    setRowData((pre) => ({
      ...pre,
      length: data.length,
      data: rowData.template_cd + " : " + rowData.descs,
    }));
    if (selectedRowKeys && selectedRowKeys === rowData.template_cd) {
      onRowClick(data.length, rowData.template_cd + " : " + rowData.template_descs, 1);
    }

    setSelectedRowKeys(rowData.template_cd);
  };
          ...
          <div class="container Ordertbl">
            <MaterialTable
              ...
              options={{
                ...
                rowStyle: (rowData) => ({
                  backgroundColor:
                    rowData.template_cd === selectedRowKeys ? "#a9ebb9" : "#FFF",
