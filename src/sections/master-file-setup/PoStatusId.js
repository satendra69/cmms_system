...
const PoStatusIdList = ({ onRowClick, onChangePage, onSearchChange }) => {
  ...
  useEffect(() => {
    httpCommon
      .get("/PoStatusId.php?site_cd=" + site_ID)
      .then((response) => {
  ...
  const columns = [
    {
      title: "Status",
      field: "puo_sts_status",
      width: 450,
      render: (rowData) => renderIcon(rowData),
    },
    { title: "Status Type", field: "puo_sts_status_type", width: 40 },
    {
      title: "Description",
      field: "puo_sts_description",
      width: 40,
    },
    {
      title: "Email Status",
      field: "puo_sts_email_flag",
      width: 40,
    },
  ];

  const renderIcon = (rowData) => {
    if (rowData.puo_sts_status === selectedRowKeys) {
      return (
        <div>
          <Iconify
          ...
          />
          {rowData.puo_sts_status}
        </div>
      );
    }
    return <div>{rowData.puo_sts_status}</div>;
  };
  const handleRowClick = (event, rowData) => {
    setRowData((pre) => ({
      ...pre,
      length: data.length,
      data: rowData.puo_sts_status + " : " + rowData.puo_sts_description,
    }));
    if (selectedRowKeys && selectedRowKeys === rowData.puo_sts_status) {
      onRowClick(
        data.length,
        rowData.puo_sts_status + " : " + rowData.puo_sts_description,
        1
      );
    }

    setSelectedRowKeys(rowData.puo_sts_status);
  };
  ...
  const handleSearch = (searchText) => {
    ...
    const filteredData = data.filter(
      (item) =>
        item.puo_sts_status.toLowerCase().includes(searchText.toLowerCase()) ||
          ...
          <div class="container Ordertbl">
            <MaterialTable
              ...
              options={{
                ...
                rowStyle: (rowData) => ({
                  backgroundColor:
                    rowData.puo_sts_status === selectedRowKeys
                      ? "#a9ebb9"
                      : "#FFF",
