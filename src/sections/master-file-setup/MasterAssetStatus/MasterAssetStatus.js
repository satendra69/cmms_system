...
import UserGroupADDDialog from "./MasterAssetStatusADDDialog";
import { Button, MenuItem, Typography } from "@mui/material";
import UserGroupHeadCustom from "./MasterAssetStatusHeadCustom";
import ExportUserGroupToExcel from "./ExportToExcel/ExportUserGroupToExcel";
import CustomPopover from "src/components/custom-popover/custom-popover";
import MasterAssetTypeRow from "./MasterAssetStatusRow";
import MasterAssetStatusHeadCustom from "./MasterAssetStatusHeadCustom";
import MasterAssetStatusRow from "./MasterAssetStatusRow";
import MasterAssetStatusADDDialog from "./MasterAssetStatusADDDialog";

// ----------------------------------------------------------------------

const TABLE_HEAD = [
  { id: "id", label: "Action", width: 20 },
  { id: "ast_sts_typ_cd", label: "Status Type", width: 150 },
  { id: "ast_sts_status", label: "Status", width: 100 },
  { id: "ast_sts_desc", label: "Description", width: 100 },
  { id: "ast_sts_count_dwn_time", label: "Count Down Time", width: 100 },
  { id: "ast_sts_disable_flag", label: "Disable", width: 100 },
];
...
export default function MasterAssetStatus() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await httpCommon.get(
        "/get_master_asset_status.php?site_cd=" + site_ID
      );
  ...
  const handelSearchButton = async () => {
    ...
    const filteredData = tableData.filter((item) => {
      return item.ast_sts_typ_cd.toLowerCase().includes(searchTerm) || item.ast_sts_status.toLowerCase().includes(searchTerm) || item.ast_sts_desc.toLowerCase().includes(searchTerm) || item.ast_sts_typ_cd.toLowerCase().includes(searchTerm) 
    });
