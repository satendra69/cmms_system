...
const ExportUserGroupToExcel = ({ tableData }) => {
  if (tableData !== "" && tableData !== undefined) {
    ...
    const worksheet = workbook.addWorksheet("Asset Status List");

    // Customize header names and set individual column widths
    worksheet.columns = [
      { header: "Status Type", key: "ast_sts_typ_cd", width: 15 },
      { header: "Status", key: "ast_sts_status", width: 40 },
      { header: "Description", key: "ast_sts_desc", width: 15 },
      { header: "Count Down Time", key: "ast_sts_count_dwn_time", width: 15 },
      { header: "Disable", key: "ast_sts_disable_flag", width: 15 },
    ];
    ...
    workbook.xlsx.writeBuffer().then((buffer) => {
      ...
      link.download = "Asset_Status_list_Data.xlsx";
      link.click();
    });
  }
