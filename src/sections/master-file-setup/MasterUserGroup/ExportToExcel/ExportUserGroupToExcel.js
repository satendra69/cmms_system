...
const ExportUserGroupToExcel = ({ tableData }) => {
  if (tableData !== "" && tableData !== undefined) {
    ...
    const worksheet = workbook.addWorksheet("UserGroup List");

    // Customize header names and set individual column widths
    worksheet.columns = [
      { header: "User Group", key: "usr_grp_usr_grp", width: 15 },
      { header: "Description", key: "usr_grp_desc", width: 60 },
      { header: "Disable", key: "usr_grp_disable_flag", width: 15 },
    ];
    ...
    workbook.xlsx.writeBuffer().then((buffer) => {
      ...
      link.download = "User_Group_list_Data.xlsx";
      link.click();
    });
  }
