...
import UserGroupHeadCustom from "./UserGroupHeadCustom";
...
const TABLE_HEAD = [
  ...
  { id: "usr_grp_usr_grp", label: "User Group", width: 100 },
  { id: "usr_grp_desc", label: "Description", width: 400 },
  { id: "usr_grp_disable_flag", label: "Disable", width: 600 },
];
...
export default function MasterGroupList() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await httpCommon.get(
        "/getMasterFileUserGrp.php?site_cd=" + site_ID
      );
      // console.log("response_____fetcha___",response);
      setTableData(response.data.data.user_grp);
      setDataFiltered(response.data.data.user_grp);
      const dataLength = response.data.data.user_grp;
  ...
  const handelSearchButton = async () => {
    ...
    const filteredData = tableData.filter((item) => {
      const itemName = item.usr_grp_usr_grp.toLowerCase();
      const itemDesc = item.usr_grp_desc.toLowerCase();
