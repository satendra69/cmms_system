...
export default function MasterAssetCriticalADDDialog({
  open,
  handleClose,
  setRefetch,
  tableData,
    ...
    const fetchLabel = async () => {
      try {
        const response = await httpCommon.get(
          "/get_asset_code_mandatoryfilled.php"
        );


        if (response.data.status === "SUCCESS") {
          setGroupLabel(response?.data?.asset_code
            ?.MandatoryField);
        }
      } catch (error) {
        console.log(error);
...
const handleErrorToast =(msg)=>{
  ...
  const handleSubmitForm = async () => {
    ...
    } else if (!data.ast_cod_ast_cd) {
      setError2("ast_cod_ast_cd")
      handleErrorToast("Asset Code")
    } else if (!data.ast_cod_desc) {
      setError2("ast_cod_desc")
      handleErrorToast("Description")
    } else {
      const body = {
        ast_cod_disable_flag: checkData ? 1 : 0,
  ...
  setError2("");
    if (e.target.name == "ast_cod_ast_cd") {
      if (value.length < 6) {
        const res = tableData.find(
          (item) => item.ast_cod_ast_cd === value.toUpperCase()
        );
      ...
      <DialogContent sx={{ mt: 2 }} >
        ...
        <Grid container spacing={2} style={{padding:2}}>
          ...
          <Grid item xs={12} md={12}>
            <Stack spacing={1} >
              <Typography variant="subtitle2" style={{ color: "red" }}>
                {findCustomizeLabel("ast_cod_ast_cd") || "loading..."}
              ...
              <div>
                <TextField
                  ...
                  name="ast_cod_ast_cd"
                  size="small"
                  className={error2==="ast_cod_ast_cd"?"errorEmpty":""}
                  value={data.ast_cod_ast_cd}
                  onChange={handleData}
                  fullWidth
          ...
          <Grid item xs={12} md={12}>
            <Stack spacing={1}>
              <Typography variant="subtitle2" style={{ color: "red" }}>
                {findCustomizeLabel("ast_cod_desc") || "loading..."}
              </Typography>
              <div>
                <TextareaAutosize
                  ...
                  onClick={handleError}
                  value={data.ast_cod_desc}
                  onChange={handleData}
                  className={`TxtAra ${error2 === "ast_cod_desc"?"errorEmpty":""}`}
