...
export default function MasterAssetCodeRow({
  ...
  const { ast_cod_ast_cd, ast_cod_desc, ast_cod_disable_flag } = row;
  ...
  return (
    <>
      ...
      <TableRow hover selected={selected}>
        ...
        <TableCell sx={{ height: "40px" }}>{ast_cod_ast_cd}</TableCell>
        <TableCell sx={{ marginLeft: "-10px" }}>{ast_cod_desc}</TableCell>
        <TableCell>
          <div>
            <Checkbox
              checked={Number(ast_cod_disable_flag)}
              disabled
              // style={{
              //   color: "green",
              // }}
