...
function MasterAssetCodeDialog({ open, handleClose, rowData, setRefetch }) {
  const [groupLabel, setGroupLabel] = React.useState([]);
  let site_ID = localStorage.getItem("site_ID");
  const [error, setError] = React.useState(false);
  let loginUser = localStorage.getItem("emp_mst_login_id");
  const [data, setData] = React.useState(
    rowData && rowData.ast_cod_desc ? rowData.ast_cod_desc : ""
  );
  const [checkData, setCheckData] = React.useState(
    rowData && rowData.ast_cod_disable_flag ? Number(rowData.ast_cod_disable_flag) : 0
  );
  // customize label
...
const handleErrorToast =(msg)=>{
  ...
  const handleSubmitForm = async () => {
    ...
    const body = {
      ...
      ast_cod_desc: data,
      ast_cod_ast_cd: rowData.ast_cod_ast_cd,
            ...
            <Stack spacing={1}  >
              ...
              <div>
                <TextField
                  ...
                  disabled
                  value={
                    rowData && rowData.ast_cod_ast_cd
                      ? rowData.ast_cod_ast_cd
                      : ""
                  }
                  fullWidth
                  placeholder=""
...
export default MasterAssetCodeDialog;
