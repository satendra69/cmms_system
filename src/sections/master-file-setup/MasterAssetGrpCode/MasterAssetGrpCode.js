...
import MasterAssetGrpADDDialog from "./MasterAssetGrpADDDialog";
import { Button, MenuItem, Typography } from "@mui/material";
import MasterAssetGrpHeadCustom from "./MasterAssetGrpHeadCustom";
import ExportUserGroupToExcel from "./ExportToExcel/ExportUserGroupToExcel";
import CustomPopover from "src/components/custom-popover/custom-popover";
import MasterAssetGrpRow from "./MasterAssetGrpRow";

// ----------------------------------------------------------------------

const TABLE_HEAD = [
  { id: "id", label: "Action", width: 20 },
  { id: "ast_grp_grp_cd", label: "Group Code", width: 130 },
  { id: "ast_grp_desc", label: "Description", width: 200 },
  { id: "ast_grp_option", label: "Auto Number", width: 130 },
  { id: "ast_grp_serial", label: "Generate Serialize Stock", width: 250 },
  { id: "ast_grp_disable_flag", label: "Disable", width: 130 },
  { id: "ast_grp_separator", label: "Seprator", width: 130 },
  { id: "ast_grp_counter", label: "Counter", width: 130 },
  { id: "Sample_Format", label: "Sample Format", width: 150 },
];
...
export default function MasterAssetGrpCode() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await httpCommon.get(
        "/get_master_asset_grp_code.php?site_cd=" + site_ID
      );
      // console.log("response_____fetcha___",response.data);
  ...
  const handelSearchDebounce = async () => {
    ...
    const filteredData = tableData.filter((item) => {
      const ast_grp_grp_cd = item.ast_grp_grp_cd.toLowerCase();
      const ast_grp_desc = item.ast_grp_desc.toLowerCase();
      
      const Sample_Format = item.Sample_Format.toLowerCase();


      return ast_grp_grp_cd.includes(searchTerm) || ast_grp_desc.includes(searchTerm) ||  Sample_Format.includes(searchTerm) ;
    });
