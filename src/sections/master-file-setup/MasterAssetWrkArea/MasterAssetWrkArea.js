...
import MasterAssetWrkHeadCustom from "./MasterAssetWrkHeadCustom"

import MasterAssetWrkADDDialog from "./MasterAssetWrkADDDialog";
import { Button, MenuItem, Typography } from "@mui/material";

import ExportUserGroupToExcel from "./ExportToExcel/ExportUserGroupToExcel";
import CustomPopover from "src/components/custom-popover/custom-popover";
import MasterAssetWrkRow from "./MasterAssetWrkRow";

// ----------------------------------------------------------------------

const TABLE_HEAD = [
  { id: "id", label: "Action", width: 20 },
  { id: "mst_war_work_area", label: "Work Area", width: 130 },
  { id: "mst_war_desc", label: "Description", width: 200 },
  { id: "mst_war_disable_flag", label: "Disable", width: 600 },
];
...
export default function MasterAssetWrkArea() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await httpCommon.get(
        "/get_master_wrk_area.php?site_cd=" + site_ID
      );
  ...
  const handelSearchDebounce = async () => {
    ...
    const filteredData = tableData.filter((item) => {
      const mst_war_work_area = item.mst_war_work_area.toLowerCase();
      const mst_war_desc = item.mst_war_desc.toLowerCase();
      return mst_war_work_area.includes(searchTerm) || mst_war_desc.includes(searchTerm);
    });
