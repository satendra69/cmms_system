...
const ExportUserGroupToExcel = ({ tableData }) => {
  if (tableData !== "" && tableData !== undefined) {
    ...
    const worksheet = workbook.addWorksheet("Asset Work Area");

    // Customize header names and set individual column widths
    worksheet.columns = [
      { header: "Work Area", key: "mst_war_work_area", width: 15 },
      { header: "Description", key: "mst_war_desc", width: 40 },
      { header: "Disable", key: "mst_war_disable_flag", width: 15 },
    ];
    ...
    workbook.xlsx.writeBuffer().then((buffer) => {
      ...
      link.download = "Asset_Wrk_area_Data.xlsx";
      link.click();
    });
  }
