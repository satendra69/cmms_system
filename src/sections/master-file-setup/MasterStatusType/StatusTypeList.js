...
import GroupTableRow from "./StatusTypeRow";
...
const TABLE_HEAD = [
  ...
  { id: "sts_typ_cat_cd", label: "Category Code", width: 20, padding: 10 },
  { id: "sts_typ_desc", label: "Description", width: 170, padding: 10 },
  { id: "sts_typ_typ_cd", label: "Category Type", width: 500, padding: 10 },
];
...
export default function StatusTypeList() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await httpCommon.get(
        "/get_master_status_type.php?site_cd=" + site_ID
      );
      setTableData(response.data.sts_typ);
      setDataFiltered(response.data.sts_typ);
      const dataLength = response.data.sts_typ;
  ...
  const handelSearchButton = async () => {
    ...
    const filteredData = tableData.filter((item) => {
      const itemName = item.sts_typ_cat_cd.toLowerCase();
      const itemDesc = item.sts_typ_desc.toLowerCase();
      const itemtypeCd = item.sts_typ_typ_cd.toLowerCase();
      return (
        itemName.includes(searchTerm) ||
        itemDesc.includes(searchTerm) ||
        itemtypeCd.includes(searchTerm)
      );
    });
  ...
  return (
    <>
      <Helmet>
        <title>Master Status Type</title>
        <meta name="description" content="Master Status Type" />
      </Helmet>
      <Container maxWidth={settings.themeStretch ? false : "lg"}>
        <div className="CustomBreadAsset">
          <CustomBreadcrumbs
            heading="Master Status Type"
            links={[]}
            sx={{ mb: { xs: 3, md: 5 } }}
          />
        </div>
