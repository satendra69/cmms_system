...
import MasteLevelADDDialog from "./MasteLevelADDDialog";
...
import MasterAssetLevelRow from "./MasterAssetLevelRow";

// ----------------------------------------------------------------------

const TABLE_HEAD = [
  { id: "id", label: "Action", width: 20 },
  { id: "ast_lvl_ast_lvl", label: "Asset Level", width: 130 },
  { id: "ast_lvl_desc", label: "Description", width: 200 },
  { id: "ast_lvl_disable_flag", label: "Disable", width: 600 },
];
...
export default function MasterAssetLevelDialog() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await httpCommon.get(
        "/get_master_level.php?site_cd=" + site_ID
      );
  ...
  const handelSearchDebounce = async () => {
    ...
    const filteredData = tableData.filter((item) => {
      const ast_lvl_ast_lvl = item.ast_lvl_ast_lvl.toLowerCase();
      const ast_lvl_desc = item.ast_lvl_desc.toLowerCase();
      return ast_lvl_ast_lvl.includes(searchTerm) || ast_lvl_desc.includes(searchTerm);
    });
    setDataFiltered(filteredData);
  };
