...
const ExportUserGroupToExcel = ({ tableData }) => {
  if (tableData !== "" && tableData !== undefined) {
    ...
    const worksheet = workbook.addWorksheet("Asset Type List");

    // Customize header names and set individual column widths
    worksheet.columns = [
      { header: "Type Code", key: "ast_type_cd", width: 15 },
      { header: "Description", key: "ast_type_descs", width: 40 },
      { header: "Disable", key: "ast_type_disable_flag", width: 15 },
    ];
    ...
    workbook.xlsx.writeBuffer().then((buffer) => {
      ...
      link.download = "Asset_Type_list_Data.xlsx";
      link.click();
    });
  }
