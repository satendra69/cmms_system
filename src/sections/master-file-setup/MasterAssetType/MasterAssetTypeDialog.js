...
function UserGroupDialog({ open, handleClose, rowData, setRefetch }) {
  ...
  const [data, setData] = React.useState(
    rowData && rowData.ast_type_descs ? rowData.ast_type_descs : ""
  );
  const [checkData, setCheckData] = React.useState(
    rowData && rowData.ast_type_disable_flag ? Number(rowData.ast_type_disable_flag) : 0
  );
  // customize label
...
const handleErrorToast =(msg)=>{
  ...
  const handleSubmitForm = async () => {
    ...
    const body = {
      ...
      ast_type_descs: data,
      ast_type_cd: rowData.ast_type_cd,
    ...
    try {
      ...
      const response = await httpCommon.post(
        `/update_new_master_asset_type.php`,
        body
      );
