...
import UserGroupDialog from "./MasterAssetTypeDialog";
...
export default function MasterAssetTypeRow({
  row,
  selected,
  onSelectRow,
  onDeleteRow,
  ...
  const { ast_type_cd, ast_type_descs, ast_type_disable_flag } = row;
      ...
      if (result.isConfirmed) {
        try {
          const response = await httpCommon.post(
            "/delete_new_master_asset_type.php",
            {
  ...
  return (
    <>
      ...
      <TableRow hover selected={selected}>
        ...
        <TableCell sx={{ height: "40px" }}>{ast_type_cd}</TableCell>
        <TableCell sx={{ marginLeft: "-10px" }}>{ast_type_descs}</TableCell>
        <TableCell>
          <div>
            <Checkbox
              checked={Number(ast_type_disable_flag)}
              disabled
              // style={{
              //   color: "green",
              // }}
