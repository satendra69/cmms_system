...
import UserGroupADDDialog from "./MasterAssetTypeADDDialog";
...
import MasterAssetTypeRow from "./MasterAssetTypeRow";

// ----------------------------------------------------------------------

const TABLE_HEAD = [
  { id: "id", label: "Action", width: 20 },
  { id: "ast_type_cd", label: "Type Code", width: 100 },
  { id: "ast_type_descs", label: "Description", width: 400 },
  { id: "ast_type_disable_flag", label: "Disable", width: 600 },
];
...
export default function MasterAssetType() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await httpCommon.get(
        "/get_master_asset_type.php?site_cd=" + site_ID
      );
      // console.log("response_____fetcha___",response.data);
      setTableData(response.data.data.asset_type);
      setDataFiltered(response.data.data.asset_type);
      const dataLength = response.data.data.asset_type;
  ...
  const handelSearchButton = async () => {
    ...
    const filteredData = tableData.filter((item) => {
      const itemName = item.ast_type_cd.toLowerCase();
      const itemDesc = item.ast_type_descs.toLowerCase();
