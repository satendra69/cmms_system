    ...
    const fetchLabel = async () => {
      try {
        const response = await httpCommon.get(
          "/get_asset_type_mandatoryfiled.php"
        );
...
const handleErrorToast =(msg)=>{
  ...
  const handleSubmitForm = async () => {
    ...
    } else if (!data.ast_type_cd) {
      setError2("ast_type_cd")
      handleErrorToast("Type Code")
    } else if (!data.ast_type_descs) {
      handleErrorToast("Description")
      setError2("ast_type_descs")
    } else {
      const body = {
        ast_type_disable_flag: checkData ? 1 : 0,
        ast_type_descs: data.ast_type_descs,
        ast_type_cd: data.ast_type_cd,
        site_cd: site_ID,
     ...
     if(!missingField){
      try {
        ...
        const response = await httpCommon.post(
          `/insert_new_master_asset_type.php`,
          body
        );
...
const handleError = ()=>{
  ...
  const handleData = (e) => {
    ...
    if (e.target.name == "ast_type_cd") {
      if (value.length < 5) {
        const res = tableData.find(
          (item) => item.ast_type_cd === value.toUpperCase()
        );
      ...
      <DialogContent sx={{ mt: 2 }} >
        ...
        <Grid container spacing={2} style={{padding:2}}>
          ...
          <Grid item xs={12} md={12}>
            <Stack spacing={1} >
              <Typography variant="subtitle2" style={{ color: "red" }}>
                {findCustomizeLabel("ast_type_cd") || "loading..."}
          ...
          <Grid item xs={12} md={12}>
            <Stack spacing={1}>
              <Typography variant="subtitle2" style={{ color: "red" }}>
                {findCustomizeLabel("ast_type_descs") || "loading..."}
              </Typography>
              <div>
                <TextareaAutosize
                 ...
                 disabled={diableToggle ? true : false}
                  value={data.ast_type_descs}
                  onChange={handleData}
                  onClick={handleError}
                  className={`TxtAra ${error2 === "ast_type_descs"?"errorEmpty":""}`}
          ...
          <Grid item xs={12} md={6} mt={-2}>
            <Stack spacing={1} sx={{ pb: 1.5 }}>
              <FormControlLabel
                control={
                  <Checkbox onChange={(e) => setCheckData(e.target.checked)}   disabled={diableToggle ? true : false}
                  onClick={handleError}
                  />
                }
                label="Disable"
