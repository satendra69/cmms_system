...
import MasteLocatonADDDialog from "./MasteLocatonADDDialog";
import { Button, MenuItem, Typography } from "@mui/material";
import MasterAssetLevelHeadCustom from "./MasterAssetLevelHeadCustom";
import ExportUserGroupToExcel from "./ExportToExcel/ExportUserGroupToExcel";
import CustomPopover from "src/components/custom-popover/custom-popover";
import MasterAssetLocationRow from "./MasterAssetLocationRow";

// ----------------------------------------------------------------------

const TABLE_HEAD = [
  { id: "id", label: "Action", width: 20 },
  { id: "ast_loc_ast_loc", label: "Asset Location", width: 160 },
  { id: "ast_loc_desc", label: "Description", width: 200 },
  { id: "ast_loc_wr_option", label: "WR Option", width: 130 },
  { id: "ast_loc_wr_prefix", label: "WR Prefix", width: 130 },
  { id: "ast_loc_wr_counter", label: "WR Counter", width: 130 },
  { id: "ast_loc_wo_option", label: "WO Option", width: 130 },
  { id: "ast_loc_wo_prefix", label: "WO Prefix", width: 130 },
  { id: "ast_loc_wo_counter", label: "WO Counter", width: 130 },
  { id: "ast_loc_pm_option", label: "PM WO Option", width: 160 },
  { id: "ast_loc_pm_prefix", label: "PM WO Prefix", width: 160 },
  { id: "ast_loc_pm_counter", label: "PM WO Counter", width: 160 },
  { id: "ast_loc_disable_flag", label: "Disabled", width: 200 },
...
export default function MasterAssetLocation() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await httpCommon.get(
        "/get_master_asset_location.php?site_cd=" + site_ID
      );
      console.log("response_All",response)
      
      setTableData(response.data.data);
      setDataFiltered(response.data.data);
      const dataLength = response.data.data;
  ...
  const handelSearchDebounce = async () => {
    ...
    const filteredData = tableData.filter((item) => {
      const ast_loc_ast_loc = item.ast_loc_ast_loc.toLowerCase();
      const ast_loc_desc = item.ast_loc_desc.toLowerCase();
      const ast_loc_wr_counter = item.ast_loc_wr_counter.toLowerCase();
      const ast_loc_wo_counter = item.ast_loc_wo_counter.toLowerCase();
      const ast_loc_pm_counter = item.ast_loc_pm_counter.toLowerCase();

      return ast_loc_ast_loc.includes(searchTerm) || ast_loc_desc.includes(searchTerm) || ast_loc_wr_counter.includes(searchTerm) || ast_loc_wo_counter.includes(searchTerm)  || ast_loc_pm_counter.includes(searchTerm) ;
    });
    setDataFiltered(filteredData);
  };

  const handelSearchButton = _.debounce(handelSearchDebounce, 500);

  const handleClickNew = (e, result) => {
    if (result !== "backdropClick") setDialogNew(!dialogNew);
  };

  return (
    <>
      <MasteLocatonADDDialog
        open={dialogNew}
                    ...
                    {isLoading ? ( // Assuming isLoading is a boolean state indicating whether data is loading
                      ...
                      <>
                        {tableData.length === 0 ? (
                          ...
                          <>
                            {/* Data mapping */}
                            {console.log("rowperPage", table.rowsPerPage)}
                            {dataFiltered
                              .slice(
