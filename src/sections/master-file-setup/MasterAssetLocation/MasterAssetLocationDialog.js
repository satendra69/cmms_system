...
export default function MasterAssetLocationDialog({
  open,
  handleClose,
  setRefetch,
  rowData
  ...
  const [data, setData] = React.useState({
    ast_loc_ast_loc: rowData && rowData.ast_loc_ast_loc?rowData.ast_loc_ast_loc:"",
    ast_loc_desc:rowData && rowData.ast_loc_desc?rowData.ast_loc_desc:"",
    ast_loc_wr_prefix: rowData && rowData.ast_loc_wr_prefix?rowData.ast_loc_wr_prefix:"",
    ast_loc_wr_counter: rowData && rowData.ast_loc_wr_counter?rowData.ast_loc_wr_counter:"",
    ast_loc_wo_prefix: rowData && rowData.ast_loc_wo_prefix?rowData.ast_loc_wo_prefix:"",
    ast_loc_wo_counter: rowData && rowData.ast_loc_wo_counter?rowData.ast_loc_wo_counter:"",
    ast_loc_pm_prefix: rowData && rowData.ast_loc_pm_prefix?rowData.ast_loc_pm_prefix:"",
    ast_loc_pm_counter: rowData && rowData.ast_loc_pm_counter?rowData.ast_loc_pm_counter:"",
  });

  const [checkData, setCheckData] = React.useState({
    ast_loc_wr_option:rowData && rowData.ast_loc_wr_option?rowData.ast_loc_wr_option: 0,
    ast_loc_wo_option: rowData && rowData.ast_loc_wo_option?rowData.ast_loc_wo_option: 0,
    ast_loc_pm_option: rowData && rowData.ast_loc_pm_option?rowData.ast_loc_pm_option: 0,
    ast_loc_disable_flag: rowData && rowData.ast_loc_disable_flag?rowData.ast_loc_disable_flag: 0,
  });

  const [counter,setCounter] = React.useState({
    wrCounter:rowData && rowData.ast_loc_wr_counter?rowData.ast_loc_wr_counter : "100001",
    woCounter:rowData && rowData.ast_loc_wo_counter?rowData.ast_loc_wo_counter : "100001",
    pmCounter:rowData && rowData.ast_loc_pm_counter?rowData.ast_loc_pm_counter : "100001",
  })
  ...
  const handleSubmitForm = async () => {
    if (error) {
      ...
      const body = {
       ...checkData,
       ...data,
       ast_loc_wr_counter:counter && counter.wrCounter?counter.wrCounter:"100001",
       ast_loc_wo_counter:counter && counter.woCounter?counter.woCounter:"",
       ast_loc_pm_counter:counter && counter.pmCounter?counter.pmCounter:"",
       site_cd:site_ID,
      ...
      if (!missingField) {
        try {
          const response = await httpCommon.post(
            `/update_master_asset_locaton.php`,
            body
          );
      ...
      <DialogContent sx={{ mt: 2 }}>
            ...
            <Stack spacing={1}>
              ...
              <div>
                <TextareaAutosize
                  ...
                  onChange={handleData}
                  className={`TxtAra ${error2 === "ast_loc_desc"?"errorEmpty":""}`}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "5px",padding:"10px 0"  }}>
          <Icon
            ...
            style={{ color: "gray" }}
          />
          <Typography style={{ fontWeight: "600",color:"gray" }}>Auto Number</Typography>
          <Divider/>
        </div>
...
<div style={{padding:"10px 0"}}>
  <div className="shadowColor" style={{padding:"10px"}}>
        ...
        >
          ...
          <Grid item md={4} sm={6}>
          <Typography style={{ fontWeight: "600",fontSize:"14px" }}>Work Request</Typography>

          <Stack spacing={1} sx={{ pb: 0.5 }}>
              <Typography style={{fontWeight:"600",fontSize:"13px",display:"flex",alignItems:"center"}}
              className={findCustomizerequiredLabel("ast_loc_wr_option")?"red":""}
            
              
              >{findCustomizeLabel("ast_loc_wr_option") || "loading..."}   <Checkbox
                    onChange={(e) => setCheckData((pre)=>({
                      ...pre,ast_loc_wr_option:e.target.checked
                    }))}
                    disabled={diableToggle ? true : false}
                    onClick={handleError}
          ...
          <Stack spacing={1} sx={{ pb: 0.5 }}>
          <Typography style={{fontWeight:"600",fontSize:"13px",display:"flex",alignItems:"center"}}
              className={findCustomizerequiredLabel("ast_loc_wr_prefix")?"red":""}
              >
              ...
              <div>
                <TextField
                  ...
                  className={`${error2 === "ast_loc_wr_prefix"?"errorEmpty":"" }`}
                  onChange={handleData}
                  fullWidth
                  inputProps={{ maxLength: 50 }}
                />
                  ...
                  <Stack spacing={1} sx={{ pb: 0.5 }}>
                  <Typography style={{fontWeight:"600",fontSize:"13px",display:"flex",alignItems:"center"}}
              className={findCustomizerequiredLabel("ast_loc_wr_counter")?"red":""}
              >
          ...
          <Grid item md={4} sm={6} >
          <Typography style={{ fontWeight: "600",fontSize:"14px",whiteSpace:"nowrap" }}>Corrective Work Order</Typography>

          <Stack spacing={1} sx={{ pb: 0.5 }}>
          <Typography style={{ fontSize: "14px", fontWeight: "600" }}
                   className={findCustomizerequiredLabel("ast_loc_wo_option")?"red":""}
                >
          ...
          <Stack spacing={1} sx={{ pb: 0.5 }}>
          <Typography variant="subtitle2" className={findCustomizerequiredLabel("ast_loc_wo_prefix")?"red":""} >
              ...
              <div>
                <TextField
                  ...
                  size="small"
                  className={`${error2 === "ast_loc_wo_prefix"?"errorEmpty":"" }`}
                ...
                />
                  ...
                  <Stack spacing={1} sx={{ pb: 0.5 }}>
          <Typography variant="subtitle2" className={findCustomizerequiredLabel("ast_loc_wo_counter")?"red":""}>
          ...
          <Grid item md={4} sm={6}>
          <Typography style={{ fontWeight: "600",fontSize:"14px",whiteSpace:"nowrap" }}>Premetive Work Order</Typography>

          <Stack spacing={1} sx={{ pb: 0.5 }}>

          <Typography variant="subtitle2" className={findCustomizerequiredLabel("ast_loc_pm_option")?"red":""}>
          ...
          <Stack spacing={1} sx={{ pb: 0.5 }}>
          <Typography variant="subtitle2"  className={findCustomizerequiredLabel("ast_loc_pm_prefix")?"red":""}>
             {findCustomizeLabel("ast_loc_pm_prefix") || "PM WO Prefix"}
              ...
              <div>
                <TextField
                  ...
                  size="small"
                  className={`${error2 === "ast_loc_pm_prefix"?"errorEmpty":"" }`}
