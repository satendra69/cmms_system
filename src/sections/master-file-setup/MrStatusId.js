...
const MrStatusIdList = ({ onRowClick, onChangePage, onSearchChange }) => {
  ...
  useEffect(() => {
    httpCommon
      .get("/MrStatusId.php?site_cd=" + site_ID)
      .then((response) => {
  ...
  const columns = [
    {
      title: "Status",
      field: "mtr_sts_status",
      width: 450,
      render: (rowData) => renderIcon(rowData),
    },
    { title: "Status Type", field: "mtr_sts_status_type", width: 40 },
    {
      title: "Description",
      field: "mtr_sts_description",
      width: 40,
    },
    {
      title: "Email Status",
      field: "mtr_sts_email_flag",
      width: 40,
    },
  ];

  const renderIcon = (rowData) => {
    if (rowData.mtr_sts_status === selectedRowKeys) {
      return (
        <div>
          <Iconify
          ...
          />
          {rowData.mtr_sts_status}
        </div>
      );
    }
    return <div>{rowData.mtr_sts_status}</div>;
  };
  const handleRowClick = (event, rowData) => {
    setRowData((pre) => ({
      ...pre,
      length: data.length,
      data: rowData.mtr_sts_status + " : " + rowData.mtr_sts_description,
    }));
    if (selectedRowKeys && selectedRowKeys === rowData.mtr_sts_status) {
      onRowClick(
        data.length,
        rowData.mtr_sts_status + " : " + rowData.mtr_sts_description,
        1
      );
    }

    setSelectedRowKeys(rowData.mtr_sts_status);
  };
  ...
  const handleSearch = (searchText) => {
    ...
    const filteredData = data.filter(
      (item) =>
        item.mtr_sts_status.toLowerCase().includes(searchText.toLowerCase()) ||
          ...
          <div class="container Ordertbl">
            <MaterialTable
              ...
              options={{
                ...
                rowStyle: (rowData) => ({
                  backgroundColor:
                    rowData.mtr_sts_status === selectedRowKeys
                      ? "#a9ebb9"
                      : "#FFF",
