...
import ExportUserGroupToExcel from "./ExportToExcel/ExportCurrencyList";
import CustomPopover from "src/components/custom-popover/custom-popover";

import CurrencyHeadCustom from "./CurrencyHeadCustom";
...
const TABLE_HEAD = [
  { id: "id", label: "Action", width: 0 },
  { id: "cur_mst_cur_code", label: "Currency Code", width: 120 },
  { id: "cur_mst_desc", label: "Currency Description", width: 150 },
  { id: "cur_mst_label", label: "Currency Label", width: 100 },
  { id: "cur_mst_base_cur", label: "Base Currency", width: 120 },
  { id: "cur_mst_exchange_rate", label: "Exchange Rate", width: 60 },
  { id: "cur_mst_exchange_rate_date", label: "Exchange Rate Date", width: 130 },
  { id: "cur_mst_format_string", label: "Format String", width: 20 },
];
...
export default function CurrencyList() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await httpCommon.get(
        "/get_master_currency_code.php?site_cd=" + site_ID
      );

      setTableData(response.data.Currency_Code);
      setDataFiltered(response.data.Currency_Code);
      const dataLength = response.data.Currency_Code;
  ...
  const handelSearchButton = async () => {
    ...
    const filteredData = tableData.filter((item) => {
      const itemName = item.cur_mst_cur_code.toLowerCase();
      const itemDesc = item.cur_mst_desc.toLowerCase();
      const label = item.cur_mst_label.toLowerCase();
      const base = item.cur_mst_base_cur.toLowerCase();
      const exchange_rate = item.cur_mst_exchange_rate.toLowerCase();
      const format_string = item.cur_mst_format_string.toLowerCase();
      ...
      return (
        ...
        exchange_rate.includes(searchTerm) ||
        format_string.includes(searchTerm)
      );
    });
  ...
  return (
    <>
      ...
      <Helmet>
        ...
        <meta name="description" content="Currency Code" />
      </Helmet>
