...
const handleError = ()=>{
  ...
  const handleSubmitForm = async () => {
    ...
    } else if (!data.ast_cri_cri_factor) {
      handleErrorToast("Critical Factor")
      setError2("ast_cri_cri_factor")

    } else if (!data.ast_cri_desc) {
      handleErrorToast("Description")
      setError2("ast_cri_desc")
    } else {
      const body = {
     ...
     if(!missingField){
      try {
        ...
        const response = await httpCommon.post(
          `/insert_master_critical_factor.php`,
          body
        );
  ...
  setError2("")
    ...
    if (e.target.name == "ast_cri_cri_factor") {
      if (value.length < 2) {
        const res = tableData.find(
          (item) => item.ast_cri_cri_factor === value.toUpperCase()
        );
            ...
            <Stack spacing={1} >
              ...
              <div>
                <TextField
                  id="outlined-basic"
                  variant="outlined"
                  name="ast_cri_cri_factor"
                  size="small"
                  className={error2==="ast_cri_cri_factor"?"errorEmpty":""}
                  value={data.ast_cri_cri_factor}
                  onChange={handleData}
                  fullWidth
            ...
            <Stack spacing={1}>
              ...
              <div>
                <TextareaAutosize
                 ...
                 onClick={handleError}
                  value={data.ast_cri_desc}
                  onChange={handleData}
                  className={`TxtAra ${error2==="ast_cri_desc"?"errorEmpty":""}`}
