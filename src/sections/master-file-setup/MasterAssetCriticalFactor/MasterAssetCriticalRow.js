...
import MasterAssetCriticalDialog from "./MasterAssetCriticalDialog";
...
export default function MasterAssetTypeRow({
  ...
  const { ast_cri_cri_factor, ast_cri_desc, ast_cri_disable_flag } = row;
      ...
      if (result.isConfirmed) {
        try {
          const response = await httpCommon.post(
            "/delete_new_master_asset_cri.php",
            {
  ...
  return (
    <>
      ...
      <TableRow hover selected={selected}>
        ...
        <TableCell sx={{ height: "40px" }}>{ast_cri_cri_factor}</TableCell>
        <TableCell sx={{ marginLeft: "-10px" }}>{ast_cri_desc}</TableCell>
        <TableCell>
          <div>
            <Checkbox
              checked={Number(ast_cri_disable_flag)}
              disabled
              // style={{
              //   color: "green",
              // }}
