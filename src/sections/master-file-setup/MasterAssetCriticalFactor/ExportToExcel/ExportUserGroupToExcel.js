...
const ExportUserGroupToExcel = ({ tableData }) => {
  if (tableData !== "" && tableData !== undefined) {
    ...
    const worksheet = workbook.addWorksheet("Asset Critical Factor");

    // Customize header names and set individual column widths
    worksheet.columns = [
      { header: "Crtical Factor", key: "ast_cri_cri_factor", width: 15 },
      { header: "Description", key: "ast_cri_desc", width: 40 },
      { header: "Disable", key: "ast_cri_disable_flag", width: 15 },
    ];
    ...
    workbook.xlsx.writeBuffer().then((buffer) => {
      ...
      link.download = "Asset_Critical_Factor_list_Data.xlsx";
      link.click();
    });
  }
