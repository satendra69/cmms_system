...
import MasterAssetCriticalADDDialog from "./MasterAssetCriticalADDDialog";
import { Button, MenuItem, Typography } from "@mui/material";
import UserGroupHeadCustom from "./MasterAssetTypeHeadCustom";
import ExportUserGroupToExcel from "./ExportToExcel/ExportUserGroupToExcel";
import CustomPopover from "src/components/custom-popover/custom-popover";
import MasterAssetCriticalRow from "./MasterAssetCriticalRow";

// ----------------------------------------------------------------------

const TABLE_HEAD = [
  { id: "id", label: "Action", width: 20 },
  { id: "ast_cri_cri_factor", label: "Crtical Factor", width: 130 },
  { id: "ast_cri_desc", label: "Description", width: 200 },
  { id: "ast_cri_disable_flag", label: "Disable", width: 600 },
];
...
export default function MasterAssetType() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await httpCommon.get(
        "/get_master_asset_critical_factor.php?site_cd=" + site_ID
      );
  ...
  const handelSearchDebounce = async () => {
    ...
    const filteredData = tableData.filter((item) => {
      const ast_cri_cri_factor = item.ast_cri_cri_factor.toLowerCase();
      const ast_cri_desc = item.ast_cri_desc.toLowerCase();
      return ast_cri_cri_factor.includes(searchTerm) || ast_cri_desc.includes(searchTerm);
    });
