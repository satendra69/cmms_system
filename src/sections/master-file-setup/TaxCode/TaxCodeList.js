...
import TaxCodeAddDialog from "./TaxCodeAddDialog";
import TaxCodeRows from "./TaxCodeRows";
import TaxCodeHeadCustom from "./TaxCodeHeadCustom";

// ----------------------------------------------------------------------

const TABLE_HEAD = [
  { id: "id", label: "Action", width: 0 },
  { id: "tax_mst_type", label: "Type of Tax", width: 130 },
  { id: "tax_mst_tax_cd", label: "Tax Code", width: 120 },
  { id: "tax_mst_desc", label: "Description", width: 200 },
  { id: "tax_mst_tax_rate", label: "Tax Rate", width: 120 },
  { id: "tax_mst_disable_flag", label: "Disable", width: 400 },
];
...
export default function TaxCodeList() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await httpCommon.get(
        "/get_master_tax_code.php?site_cd=" + site_ID
      );
      if (response.data.status === "SUCCESS") {
        setTableData(response.data.tax_code);
        setDataFiltered(response.data.tax_code);
        const dataLength = response.data.tax_code;
  ...
  const handelSearchButton = async () => {
    ...
    const filteredData = tableData.filter((item) => {
      const newTerm = item.tax_mst_type == "S" ? "Supply" : "Purchase";
      return (
        item.tax_mst_tax_cd.toLowerCase().includes(searchTerm) ||
        item.tax_mst_desc.toLowerCase().includes(searchTerm) ||
        item.tax_mst_tax_rate.toLowerCase().includes(searchTerm) ||
        newTerm.toLowerCase().includes(searchTerm)
      );
    });
  ...
  return (
    <>
      ...
      <Helmet>
        ...
        <meta name="description" content="Tax Code" />
      </Helmet>
