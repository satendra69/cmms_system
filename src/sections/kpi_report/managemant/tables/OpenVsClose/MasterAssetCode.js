...
import MasterAssetCodeADDDialog from "./MasterAssetCodeADDDialog";
import { Button, Icon, MenuItem, Typography } from "@mui/material";
import MasterAssetCodeHeadCustom from "./MasterAssetCodeHeadCustom";
import ExportUserGroupToExcel from "./ExportToExcel/ExportUserGroupToExcel";
import CustomPopover from "src/components/custom-popover/custom-popover";
import MasterAssetCodeRow from "./MasterAssetCodeRow";
...
const TABLE_HEAD = [
  { id: "MonthName", label: "Month", width: 20 },
  { id: "TotalWO_Open", label: "Outstanding WOs", width: 100 },
  { id: "TotalWO_Closed", label: "Closed WOs", width: 100},
  { id: "Total WOs", label: "Total WOs", width: 450},
];
...
export default function MasterAssetCode({seriesDataLoc,seriesData,setShowTable}) {
  ...
  const fetchData =async () => {
    ...
    if (response.data.status === "SUCCESS") {
      ...
      setTableData(aggregatedDataArray);
      setDataFiltered(aggregatedDataArray)
   
      setTotalRow(aggregatedDataArray.length);
...
useEffect(()=>{
  ...
  const handelSearchDebounce = async () => {
    ...
    const filteredData = tableData.filter((item) => {
      const ast_cod_ast_cd = item.ast_cod_ast_cd.toLowerCase();
      const ast_cod_desc = item.ast_cod_desc.toLowerCase();
      return ast_cod_ast_cd.includes(searchTerm) || ast_cod_desc.includes(searchTerm);
    });
    ...
    <>
      ...
      <Container maxWidth={settings.themeStretch ? false : "lg"}>
        <div className="CustomBreadAsset">
          <CustomBreadcrumbs
            heading="Open vs Closed Work Order Analysis"
            links={[]}
            action={
              <div onClick={()=>setShowTable(false)}>
        <Tooltip title="Graph View" arrow>
        <IconButton>
        <Iconify icon="mdi:graph-box-outline" color="#16a34a" height="22" width="22" />
        </IconButton>
        </Tooltip>
        </div>
        ...
        <div className="workReqpage">
          <Card>
            ...
            {/* Tabel */}
            <TableContainer sx={{ position: "relative",height:"220px",marginTop:"-10px" }} >
              <TableSelectedAction
                dense={table.dense}
