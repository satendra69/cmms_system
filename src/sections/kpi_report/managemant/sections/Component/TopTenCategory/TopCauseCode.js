...
function TopCauseCode({seriesData,seriesDataLoc}) {
  ...
  const fetchKpi = async () => {
    ...
    const response = await httpCommon.get(
      `/get_kpi_cause_code.php?site_cd=${site_ID}&year=${year}&location=${loc}`
    );
    console.log("dddjj",response.data)
    if (response.data.status === "SUCCESS") {
  ...
  useEffect(() => {
    const fetchKpi = async () => {
      ...
      const response = await httpCommon.get(
        `/get_kpi_cause_code_pie.php?site_cd=${site_ID}&sortBy=${
          sort ? sort : "Yearly"
        }&year=${year}`
      );
...
{/* adjust hieght here */}
        <div className="kpi_report" style={{ height: "300px",padding:"0 20px" }}>
        {/* <Card style={{  height: "450px",padding:"0 20px" }}> */}

            <div className="title">
              <div>
                <h3 style={{ fontStyle: "bold", marginBottom: "4px",fontSize:"13px" }}>
             Top 10   Cause Code by Corrective Work Order
                </h3>
                <Divider
                  style={{ width: "50%", height: "10px", marginTop: "-12px" }}
                />
            ...
            >
              ...
              <h2 className="vertical_text"  style={{marginTop:"40px",fontSize:"10px" }}>Total Work Order</h2>
              </div>

              <div className="chart" style={{width:"100%",marginLeft:"-50px",marginTop:"-40px"}}>
