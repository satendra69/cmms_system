...
//
import MrTableRow from './mr-table-row';
...
export default function Maintence_List() {
...
const [selectedComeBack, setSelectedComeBack] = useState(comeBack || '');
 const [selectedRowIdbackState, setSelectedRowIdbackState] = useState(selectedRowIdBack || '');
  ...
  const fetchFilterSubPopupSavedropdon = async () => {
    ...
    try {
      const response = await httpCommon.get(
        `/get_maintence_mr_filter_dropdown.php?site_cd=${site_ID}&auditUser=${AuditUser}`
      );
    ...
    const fetchDataGaugeDSB = useCallback(async () => {
      ...
      try {
        const response = await httpCommon.post(
          "/get_gauge_dashbord_material_request_data.php?page=" + currentPage,
          {
       ...
       //console.log("response_____using _dashbord",response);
        if (response.data.status === "SUCCESS") {
          if (response.data.data.result.length > 0) {
           ...
           setResponceStats(response.data.StatusPRM);
           setTotalRow(response.data.TotalCountPRM);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await httpCommon.get(
        `/get_maintence_mr_master_table_data.php?site_cd=${site_ID}&page=${currentPage}`
      );
...
const fetchDataUsingRefreshBtn = useCallback(async () =>{
  ...
  const handleDeleteRow = useCallback(async (id, row) => {
    ...
    const Rowid = id;
    const MR_no = row.col1;
    const MR_workorderNo = row.col2;
    //console.log("Rowid___row",row);
    if (Rowid !== '' && MR_no !== '') {
      Swal.fire({
        title: "Are you sure?",
        ...
        if (result.isConfirmed) {
          ...
          try {
            const response = await httpCommon.get(
              `/delete_mr_list_data.php?site_cd=${site_ID}&mst_id=${Rowid}&MR_no=${MR_no}&MR_workorderNo=${MR_workorderNo}`
            );
  ...
  const handleEditRow = useCallback(
    (id,row) => {
      ...
      if (Rowid !== '' && AstNo !== '') {
        navigate(`/dashboard/MaterialRequest/editmaterialrequest`, {
          state: {
            RowID:Rowid,
            Ast_no:AstNo,
            currentPage,
...
//console.log("rows",rows)
  ...
  const rowOptions = AssetFiledname.map((row) => ({
    ...
    const getAssetListLebel = async () => {
      try {
        const response = await httpCommon.get("/get_mtr_filled_name.php");
...
// Retrive button  funcation,setTableData
  ...
  if (DefineQueryBtn === "") {
    ...
    try {
      const response = await httpCommon.post(
        "/get_retrive_popup_mtr_filed_data.php?page=" + currentPage,
        {
          rows: rows,
          rowsort: rowsort,
          SiteCD: site_ID,
 ...
 //   console.log("retrive_all Data____");
    ...
    try {
      const response = await httpCommon.post(
        "/get_retrive_maintence_mr_all_data.php?page=" + currentPage,
        {
          SiteCD: site_ID,
          admin: emp_owner,
        }
  ...
  const InsertCf_queryListData = async () => {
    ...
    try {
      const response = await httpCommon.post(
        "/insert_mtr_filter_query_list_data.php",
        combinedData
      );
  ...
  // Query button click to funcatio start
    ...
    // console.log("rowsQrt____",rowsQrt);
        try {
          const response = await httpCommon.post(
            "/insert_mtr_query_list_save_data.php",
            combinedData
          );
...
const handlelogicalValueChangeQtr2 = (index, newValue) => {
 // setRowQtrAndset(newValue);
  ...
  const InsertCf_queryListDataSavaAs = async () => {
    ...
    try {
      const response = await httpCommon.post(
        "/insert_mtr_query_list_save_as_data.php", 
        combinedData
      );
...
const handleExportClick = () => {
  ...
  ExportMtrlistToExcel({ tableData: tableData });
  popover.onClose();
  Swal.fire({
    title: "Please Wait!",
    allowOutsideClick: false,
 ...
 // const hasRowIdValuept = rowsDropdownPrompt.length > 0 ? rowsDropdownPrompt[0].RowId : null;
  ...
  if (hasEmptyValuept) {
    ...
    try {
        ...
        const response = await httpCommon.post(
          "/insert_mtr_retrive_prompt_save_data.php",
          combinedDataPromt
        );
...
const getGroupLabel = async () => {
  try {
    const res = await httpCommon.get(
      "/get_mtr_master_mandatoryfiled.php",
    );
...
const TABLE_HEAD = [
  ...
  {
    id: "col1",
    label: CustomizeLable("mtr_mst_mtr_no") || "______",
    width: 170,
  },
  {
    id: "col2",
    label: CustomizeLable("mtr_mst_wo_no") || "______",
    width: 150,
    padding: 10,
  },
  {
    id: "col3",
    label: CustomizeLable("mtr_mst_assetno") || "_____",
    width: 120,
  },
  {
    id: "col4",
    label: CustomizeLable("mtr_mst_mr_status") || "___",
    width: 120,
  },
  {
    id: "col5",
    label: CustomizeLable("mtr_mst_org_date") || "________",
    width: 150,
  },
  {
    id: "col6",
    label: CustomizeLable("mtr_mst_status") || "_______________",
    width: 180,
  },
  {
    id: "col7",
    label: CustomizeLable("mtr_mst_tot_cost") || "_____________",
    width: 100,
  },
  {
    id: "col8",
    label: CustomizeLable("Received_Status") || "Issue Status",
    width: 140,
  },
  {
    id: "col9",
    label:
      CustomizeLable("mtr_mst_release_for_app") || "_________",
    width: 170,
  },
  {
    id: "col10",
    label: CustomizeLable("mtr_mst_email_notification") || "________",
    width: 170,
  },
  {
    id: "col11",
    label: CustomizeLable("mtr_mst_email_requestor") || "__________",
    width: 160,
  },
  {
    id: "col12",
    label: CustomizeLable("Approval Status") || "Approval_Status",
    width: 140,
  },
  {
    id: "col13",
    label: CustomizeLable("mtr_mst_req_date") || "________",
    width: 140,
  },
  {
    id: "col14",
    label: CustomizeLable("mtr_mst_costcenter") || "______",
    width: 120,
  },

  {
    id: "col15",
    label: CustomizeLable("mtr_mst_account") || "_________",
    width: 160,
  },
  {
    id: "col16",
    label: CustomizeLable("mtr_mst_entered_by") || "_______",
    width: 100,
  },

  {
    id: "col17",
    label: CustomizeLable("mtr_mst_requester") || "________",
    width: 125,
  },
  {
    id: "col18",
    label: CustomizeLable("mtr_mst_note") || "_______",
    width: 160,
  },
  {
    id: "col19",
    label: CustomizeLable("mtr_mst_create_by") || "______",
    width: 125,
  },
  {
    id: "col20",
    label: CustomizeLable("mtr_mst_create_date") || "______",
    width: 120,
  },
  {
    id: "col21",
    label: CustomizeLable("mtr_det_reject_by") || "____",
    width: 140,
  },
  {
    id: "col22",
    label: CustomizeLable("mtr_det_reject_desc") || "____",
    width: 160,
  },
  {
    id: "col23",
    label: CustomizeLable("mtr_det_reject_date") || "____",
    width: 120,
  },
  {
    id: "col24",
    label: CustomizeLable("mtr_det_approver") || "_____",
    width: 200,
  },

  {
    id: "col25",
    label: CustomizeLable("mtr_det_appr_date") || "______",
    width: 120,
  },
  {
    id: "col26",
    label: CustomizeLable("mtr_det_varchar1") || "_________",
    width: 200,
  },
  {
    id: "col27",
    label: CustomizeLable("mtr_det_varchar2") || "_________",
    width: 200,
  },
  {
    id: "col28",
    label: CustomizeLable("mtr_det_varchar3") || "_________",
    width: 200,
  },
  {
    id: "col29",
    label: CustomizeLable("mtr_det_varchar4") || "_________",
    width: 200,
  },
  {
    id: "mtr_det_varchar5",
    label: CustomizeLable("mtr_det_varchar5") || "_________",
    width: 200,
  },
  {
    id: "mtr_det_varchar6",
    label: CustomizeLable("mtr_det_varchar6") || "_________",
    width: 200,
  },
  {
    id: "mtr_det_varchar7",
    label: CustomizeLable("mtr_det_varchar7") || "_________",
    width: 200,
  },
  {
    id: "mtr_det_varchar8",
    label: CustomizeLable("mtr_det_varchar8") || "_________",
    width: 200,
  },
  {
    id: "mtr_det_varchar9",
    label: CustomizeLable("mtr_det_varchar9") || "_________",
    width: 200,
  },
  {
    id: "mtr_det_varchar10",
    label: CustomizeLable("mtr_det_varchar10") || "_________",
    width: 200,
  },
  {
    id: "mtr_det_numeric1",
    label: CustomizeLable("mtr_det_numeric1") || "_________",
    width: 200,
  },
  {
    id: "mtr_det_numeric2",
    label: CustomizeLable("mtr_det_numeric2") || "_________",
    width: 200,
  },
  {
    id: "mtr_det_numeric3",
    label: CustomizeLable("mtr_det_numeric3") || "_________",
    width: 200,
  },
  {
    id: "mtr_det_numeric4",
    label: CustomizeLable("mtr_det_numeric4") || "_________",
    width: 200,
  },
  {
    id: "mtr_det_numeric5",
    label: CustomizeLable("mtr_det_numeric5") || "_________",
    width: 200,
  },
  {
    id: "mtr_det_datetime1",
    label: CustomizeLable("mtr_det_datetime1") || "_________",
    width: 200,
  },
  {
    id: "mtr_det_datetime2",
    label: CustomizeLable("mtr_det_datetime2") || "_________",
    width: 200,
  },
  {
    id: "mtr_det_datetime3",
    label: CustomizeLable("mtr_det_datetime3") || "_________",
    width: 200,
  },
  {
    id: "mtr_det_datetime4",
    label: CustomizeLable("mtr_det_datetime4") || "_________",
    width: 200,
  },
  {
    id: "mtr_det_datetime5",
    label: CustomizeLable("mtr_det_datetime5") || "_________",
    width: 200,
  },
  {
    id: "mtr_det_note1",
    label: CustomizeLable("mtr_det_note1") || "_________",
    width: 200,
  },
  {
    id: "mtr_det_note2",
    label: CustomizeLable("mtr_det_note2") || "_________",
    width: 200,
  },
  {
    id: "mtr_det_note3",
    label: CustomizeLable("mtr_det_note3") || "_________",
    width: 200,
  },
];
...
try {
  ...
  const response = await httpCommon.post(
    `/get_maintence_mr_list_selectoption_data.php?site_cd=${site_ID}&ItemID=${selectDropRowID}&page=${page}&EmpId=${emp_owner}`
  );
  ...
  return (
    <>
          ...
          <CustomBreadcrumbs
            ...
            action={
              <Button
                ...
                to={{
                  pathname: "/dashboard/MaterialRequest/newmaterialrequest",
                  state: { select: "New_MR" },
                }}
              >
                New
                            ...
                            {dataFiltered
                                ...
                                .map((row,index) => (
                                  ...
                                  <MrTableRow
                                    ...
                                    selected={table.selected.includes(row.col49)}
                                    isHighlighted={selectedRowIdbackState && selectedRowIdbackState === row.col49} 
                                    onSelectRow={() => table.onSelectRow(row.col49)}
                                    onDeleteRow={() => handleDeleteRow(row.col49,row)}
                                    onEditRow={() => handleEditRow(row.col49,row)}
                                    onClick={() => handleRowClickTable(row.col49,row)}
      ...
      //  onClose={handleCloseQrCode}
        onClose={(event, reason) => {
          if (reason !== 'backdropClick' && reason !== 'escapeKeyDown') {
            handleCloseQrCode(event, reason);
          }
        }}
        aria-labelledby="customized-dialog-title"
        open={showQrModel}
