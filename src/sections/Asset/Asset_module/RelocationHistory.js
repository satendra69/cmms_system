  ...
  const items = Result.map((data) => ({
    ...
    cardTitle: (
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: '16px' }}>
        {/* Left Side: Work Order No and Phone Number */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {/* Work Order No */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
            <Tooltip title="Old Location Move To New Location" placement="top" arrow>
              <Iconify style={{ marginLeft: 5, cursor: 'pointer' }} icon="material-symbols-light:order-approve-outline" />
            </Tooltip>
            <Typography variant="body2" color="textSecondary" style={{ fontWeight: 'bold' }}>
              {data.ast_loc_s_asset_olocn} "Move To" {data.ast_loc_s_asset_nlocn}
            </Typography>
          </div>
  
          <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
            <Tooltip title="Audit User" placement="top" arrow>
              <Iconify style={{ marginLeft: 5, cursor: 'pointer' }} icon="majesticons:user-line" />
            </Tooltip>
            <Typography variant="body2" color="textSecondary" style={{ fontWeight: 'bold',fontSize: '12px' }}>
              {data.audit_user} {/* Adjust field name if necessary */}
            </Typography>
          </div>

          <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
            <Tooltip title="Reason" placement="top" arrow>
              <Iconify style={{ marginLeft: 5, cursor: 'pointer' }} icon="material-symbols-light:description-outline" />
            </Tooltip>
         
            <Tooltip title={data.ast_loc_s_asset_reason} placement="top" arrow>
            <Typography
              variant="body2"
              color="textSecondary"
              style={{
            ...
            >
              ...
              {data.ast_loc_s_asset_reason && data.ast_loc_s_asset_reason.length > 20 
                ? `${data.ast_loc_s_asset_reason.slice(0, 20)}...` 
                : data.ast_loc_s_asset_reason || ''}
