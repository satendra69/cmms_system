...
const AssetSpares = ({ onRowClick, data }) => {
  ...
  useEffect(() => {
    ...
    if(FormStatus !== "" && FormStatus == "NEW" ){
      get_Asset_Spares_Header(site_ID);
    }else{
  ...
  const get_Asset_Spares_Header = async (site_ID) => {
    Swal.fire({
      title: "Please Wait!",
      allowOutsideClick: false,
      customClass: {
...
const handleDelete = async (data) => {
  ...
  const addInputField = (event) => {
    ...
    inputFields.forEach((inputFields) => {
      if (inputFields.selectStock.trim() === "" || inputFields.selectStock == null) {
        Swal.fire({
          icon: "error",
          title: "Oops...",
        ...
        isValid = false;
      } else if (inputFields.setDescription.trim() === "" || inputFields.setDescription == null) {
        Swal.fire({
          icon: "error",
          title: "Oops...",
        ...
        isValid = false;
      } else if (inputFields.setPartNo === "" || inputFields.setPartNo == null) {
        Swal.fire({
          icon: "error",
          title: "Oops...",
  ...
  const handleRowPopupData = async (
    ...
    if (secondRowData == "1") {
      ...
      const stockInInputFields = inputFields.some((field) => field.selectStock === rowData);
      const stockInResult = Result.some((result) => result.ast_ls1_stock_no === rowData);
     ...
     if (stockInInputFields || stockInResult) {
      ...
      Swal.fire({
        icon: "warning",
        title: "Warning",
        text: "Duplicate Data Found Stock No.",
        customClass: {
          container: "swalcontainercustom",
        },
          });
  ...
  const PopupRowDataSelect = async (index) => {
    const stockInInputFields = inputFields.some((field) => field.selectStock === modalRowDt);
    const stockInResult = Result.some((result) => result.ast_ls1_stock_no === modalRowDt);
  ...
  const handleChange = async (index, fieldName, value, event) => {
    ...
    if (fieldName == "setQtyNeeded") {
      ...
      handleNumericInputChange_16(event, (value) => {
      ...
      list[index][fieldName] = value.slice(0, 25);
      setPartNo(value);
    }else {
      list[index][fieldName] = value;
      setInputFields(list);
  ...
  const handleShow = () => {
    ...
    if(FormStatus !== "" && FormStatus == "NEW" ){
      Swal.fire({
        ...
        title: "Please Wait!",
        allowOutsideClick: false,
        text: "Once the asset is created, you can then add detailed spares.",
        width:"400px"
      });
      onRowClick("BtnSpares");
    }else{
        setShow(true);
                        ...
                        <Grid item xs={12} md={8}>
                          <TextField
                           ...
                           // onChange={(event) => setQtyNeeded(event.target.value)}
                            onChange={(e) => handleNumericInputChange_16(e, setQtyNeeded)}
                            InputProps={{
                              inputProps: { style: { textAlign: 'right' } }
                            }}
                          />
