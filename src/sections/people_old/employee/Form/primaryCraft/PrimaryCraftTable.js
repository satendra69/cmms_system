...
const PrimaryCraftTable = ({ onRowClick, onChangePage, onSearchChange,setPrimaryCraftData }) => {
  ...
  useEffect(() => {
    httpCommon
      .get("/get_emp_primary_craft.php?site_cd=" + site_ID)
      .then((response) => {
        ...
        setData(response.data.emp_primary_craft);
        setPrimaryCraftData(response.data.emp_primary_craft)
  ...
  const columns = [
    ...
    { title: "Description", field: "crf_mst_desc", width: 40 },
    { title: "Estimate Rate", field: "crf_mst_crf_est_rate", width: 40 },
...
export default PrimaryCraftTable;
