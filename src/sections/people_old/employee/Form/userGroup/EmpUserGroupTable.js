...
const EmpUserGroupTable = ({ onRowClick, onChangePage, onSearchChange }) => {
  ...
  useEffect(() => {
    httpCommon
      .get("/get_emp_user_group.php?site_cd=" + site_ID)
      .then((response) => {
        ...
        setData(response.data.emp_usr_grp);
  ...
  const columns = [
    {
      title: "User Group",
      field: "usr_grp_usr_grp",
      width: 450,
      render: (rowData) => renderIcon(rowData),
    },
    { title: "User Description", field: "usr_grp_desc", width: 40 },
  
  ];

  const renderIcon = (rowData) => {
    if (rowData.usr_grp_usr_grp === selectedRowKeys) {
      return (
        <div>
          <Iconify
    ...
    return <div>{rowData.usr_grp_usr_grp}</div>;
  };
  const handleRowClick = (event, rowData) => {
    setRowData((pre) => ({
      ...pre,
      length: data.length,
      data: rowData.usr_grp_usr_grp + " : " + rowData.usr_grp_desc,
    }));
    if (selectedRowKeys && selectedRowKeys === rowData.usr_grp_usr_grp) {
      onRowClick(
        data.length,
        rowData.usr_grp_usr_grp + " : " + rowData.usr_grp_desc,
        1
      );
    }

    setSelectedRowKeys(rowData.usr_grp_usr_grp);
  };
          ...
          <div class="container Ordertbl">
            <MaterialTable
              ...
              options={{
                ...
                rowStyle: (rowData) => ({
                  backgroundColor:
                    rowData.usr_grp_usr_grp === selectedRowKeys
                      ? "#a9ebb9"
                      : "#FFF",
...
export default EmpUserGroupTable;
